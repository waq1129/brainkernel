% step 3 for generating brain kernel: optimize PLS with initialization from
% step 2. It provides a good PLS solution that covers both on-diagonal and
% off-diagonal covariances.

addpath(genpath(pwd)); warning off

%% load preprocessed data
load('brainkernel_step2.mat')
...
for iter = 1:niter
    ...
    lfunc = @(pp) pls(pp,input_var,pid,npid,fall_old,nf,xgrid,kernelfun,lambda,Kinv11,Kinv21,Ccovblockpp,Ccovblockpn',nsamp);
    ...
    if mod(iter,100)==0
        save(['brainkernel_step3.mat'],'-v7.3')
    end
end
save(['brainkernel_step3.mat'],'-v7.3')
