function [L,dL,fnew] = pls(pp,input_var,pid,npid,fall,nf,xgrid,kernelfun,lambda,Kinv11,Kinv21,S1,S2,nsamp)
...
df_last_beta = df_last_beta(npid,:);% = [];
dL_u1 = repmat(dL_on,nf,1);
dfu = reshape(df_last_beta,[],np);
duf = dL_u1.*dfu;
duf = sum(reshape(duf,length(npid),[]),1);
